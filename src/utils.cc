...
void InitMenuDefaults(HWND hWnd) {
  ...
  // Beziers and Lines are independent toggles (not part of the radio group
  // above). Clear first so we honour the RC regardless of the globals'
  // declared initial values — prevents "true default" from sticking if the
  // RC later changes to uncheck them.
  g_beziers = (GetMenuState(hShapes, IDM_BEZIERS, MF_BYCOMMAND) & MF_CHECKED) != 0;
  g_lines   = (GetMenuState(hShapes, IDM_LINES,   MF_BYCOMMAND) & MF_CHECKED) != 0;
...
// file (the compact codec we use for watersky.wav to keep the shipped file
...
// ("degen_art_bgm") names our open device instance so stop/close can find
// it again later.
//
...
static const wchar_t kMciBgmAlias[] = L"degen_art_bgm";
...
static std::wstring ExtractEmbeddedWavToTemp() {
  ...
  std::wstring tempPath = std::wstring(tempDir) + L"degen_art_bgm.wav";
...
bool PlayWavFile(const std::wstring& wav_file, bool use_embedded) {
  ...
  openCmd += file;
  openCmd += L"\" type waveaudio alias degen_art_bgm";
  ...
  err = mciSendStringW(L"play degen_art_bgm notify", nullptr, 0, mainHwnd);
  if (err != 0) {
    LOG(ERROR) << L"MCI play failed: " << MciErrText(err);
    mciSendStringW(L"close degen_art_bgm", nullptr, 0, nullptr);
    s_mciOpened = false;
    if (!s_embeddedTempPath.empty()) {
      DeleteFileW(s_embeddedTempPath.c_str());
      s_embeddedTempPath.clear();
...
bool StopPlayWav() {
  ...
  // restarting from 0.
  MCIERROR err = mciSendStringW(L"stop degen_art_bgm", nullptr, 0, nullptr);
  if (err != 0) {
    LOG(ERROR) << L"MCI stop failed: " << MciErrText(err);
  }
  err = mciSendStringW(L"close degen_art_bgm", nullptr, 0, nullptr);
  if (err != 0) {
